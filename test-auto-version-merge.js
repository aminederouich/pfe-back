...
// 6. Calculer les versions pour test
...
versions.forEach(version => {
  ...
  bumpTypes.forEach(bumpType => {
    try {
      // Simuler le calcul de version (sans modifier les fichiers)
      ...
      console.log(`    ${bumpType}: ${version} → ${newVersion}`);
    } catch (error) {
      console.log(`    ❌ Error calculating ${bumpType} for ${version}`);
    }
  });
});
